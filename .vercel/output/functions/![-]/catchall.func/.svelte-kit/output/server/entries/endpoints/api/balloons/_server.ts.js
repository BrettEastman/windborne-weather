...
async function GET({ url }) {
  ...
  try {
    const delay = parseInt(hourStr) * 100;
    ...
    if (errorMsg.includes("fetch")) {
      if (errorMsg.includes("ECONNREFUSED")) {
        userMessage = "Connection refused - external API may be down or blocking requests";
      } else if (errorMsg.includes("ENOTFOUND")) {
        userMessage = "DNS resolution failed - check if the API domain is accessible";
      } else if (errorMsg.includes("timeout")) {
        userMessage = "Request timed out - external API may be slow or unresponsive";
      } else {
